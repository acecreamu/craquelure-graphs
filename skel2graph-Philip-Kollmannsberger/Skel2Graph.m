function [A,node,link] = Skel2Graph(skel,THR, borderDist)
...
% mark nodes on the border (they are not real nodes)
d = borderDist;
for i=1:length(node)
    if node(i).comx < d || node(i).comx > l-d || ...
            node(i).comy < d || node(i).comy > w-d
        node(i).bord = 1;
    else
        node(i).bord = 0;
    end
